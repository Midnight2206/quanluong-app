...
description: Build or refactor a React data layer that manages server-state consumption, query and mutation orchestration, selectors, derived view state, and stable boundaries between API modules and UI components.
---
...
- Keep server cache concerns in RTK Query or another dedicated data solution when available.
...
- `react-rtk-query` for cache-first server-state management
- `react-redux-toolkit` for slice-based client state and derived selectors
...
- If RTK Query already covers the screen well, keep the extra data layer very thin.
