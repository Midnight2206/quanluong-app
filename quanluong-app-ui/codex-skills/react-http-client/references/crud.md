...
Use the shared HTTP client in service or API modules, then consume those modules from hooks, RTK Query, or feature logic.
...
- If the app already uses RTK Query, prefer RTK Query for caching and invalidation, while the shared client handles transport defaults.
