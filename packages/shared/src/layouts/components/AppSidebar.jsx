...
export function AppSidebar({ items = mainNavItems, onMobileNavActivate }) {
  ...
  return (
    <>
      ...
      <nav
        className="fixed bottom-0 left-0 right-0 z-30 flex items-stretch justify-around gap-0.5 border-t border-border bg-card/95 px-1 pb-[max(0.35rem,env(safe-area-inset-bottom))] pt-1 backdrop-blur-md print:hidden supports-[backdrop-filter]:bg-card/85 lg:hidden"
        aria-label="Điều hướng chính"
      >
        {visibleItems.map((item) => {
          const classNameBuilder = ({ isActive }) => {
            const active = navItemPathMatches(item, pathname) || isActive;
            return cn(
              "flex min-h-[3.25rem] min-w-0 flex-1 max-w-[4.5rem] flex-col items-center justify-center gap-0.5 rounded-xl px-1 py-1.5 text-[10px] font-medium leading-tight transition touch-manipulation sm:max-w-none sm:gap-1 sm:text-[11px]",
              active
                ? "bg-secondary text-foreground"
